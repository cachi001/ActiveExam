...
- [ ] 4.3 Crear `poc/k6/evidence.js`: VU que sube evidencia sintética al endpoint de ingesta, encolando re-inferencia; Done: `job_queue_depth > 0` — **PENDIENTE (decisión de diseño)**: el endpoint real `/api/v1/evidence/notify` re-descarga el binario de WORM (MinIO) + valida firma de evidencia antes de encolar (flujo pesado), y encola en la cola de PROD (no en `poc_job_queue` del worker PoC). Definir en B5: o cablear el endpoint a `PocPostgresQueue` bajo flag, o un endpoint PoC de encolado directo
