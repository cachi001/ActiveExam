...
El realm `proctoring` SHALL contener exactamente los siguientes realm roles (nombres exactos, snake_case): `estudiante`, `proctor`, `admin_examenes`, `admin_sistema`. Estos nombres coinciden con los valores del enum `Rol` del backend (`roles.py`).

#### Scenario: Roles presentes en el realm
- **WHEN** se consulta la lista de realm roles de `proctoring`
- **THEN** existen al menos `estudiante`, `proctor`, `admin_examenes`, `admin_sistema`
...
- `admin` con roles `admin_examenes` + `admin_sistema`.
...
- **THEN** `realm_access.roles` contiene `"estudiante"` y NO contiene `"proctor"` ni `"admin_examenes"`

#### Scenario: Token del admin tiene ambos roles administrativos
- **WHEN** se decodifica el access token del usuario `admin`
- **THEN** `realm_access.roles` contiene tanto `"admin_examenes"` como `"admin_sistema"`
