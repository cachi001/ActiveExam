...
- [ ] 8.2 Navegar manualmente al harness (`/admin/detection-test`), iniciar la cámara, detener y volver a iniciar — confirmar que la segunda vez el banner `'loading'` aparece brevemente o no aparece (motor cacheado) sin recargar WASM
- [ ] 8.3 Verificar que los labels de umbrales muestran nombres claros y que la edición sigue funcionando correctamente
- [ ] 8.4 Verificar que el botón "Detectar pantallas" aparece en Chrome/Edge y que el flujo de permiso funciona (conceder/denegar/reintentar)
