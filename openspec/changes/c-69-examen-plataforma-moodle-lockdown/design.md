...
1. **Backend, aditivo**: nueva migración Alembic rama slim `0026_*` (sigue `0025`), patrón aditivo de `0023` (tablas nuevas, FK con `ON DELETE CASCADE`, índices). `alembic downgrade` dropea sólo las tablas nuevas — no destructivo para el resto.
