# Spec — auth-provider-abstraction

> Capacidad de **abstracción del provider de autenticación** en el frontend: interfaz `AuthProvider` (port) con adapters `jwt` (formulario login → backend propio), `keycloak` (OIDC PKCE existente conservado), y `demo` (mock sin red). Selección por variable de entorno `VITE_AUTH_PROVIDER`.
