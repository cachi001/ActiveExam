# Spec — moodle-grade-writeback

> Devolución de la NOTA ACADÉMICA del examen a Moodle. Al finalizar la sesión de examen, el sistema calcula la nota académica (respuestas correctas) y la escribe en Moodle vía REST Web Service `core_grades_update_grades`. El disparo es server-side (cliente = sensor no confiable). Se eligió REST WS y NO LTI 1.3/AGS — eso queda como evolución futura. Lo que se escribe es la nota académica, NUNCA el score de proctoring (L2.5: el proctoring no es veredicto).
