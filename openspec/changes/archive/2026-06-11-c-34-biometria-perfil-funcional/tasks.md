...
- [x] 10.2 Verificar manualmente en Chrome desktop que los 5 retos se detectan por movimiento real de cara — QA OK, thresholds calibrados en `enrollmentChallengeDetector.ts` líneas 55-79 (corroborado en C-54/C-65 + ajustes posteriores)
- [x] 10.3 Verificar que el embedding en el log del mock API ya no es `Math.random` — QA OK: `EnrollmentBiometricStep.tsx:86` usa `computeFaceDescriptor()` de face-api (sustituido en C-56 con persistencia real)
- [x] 10.4 Verificar en Chrome mobile (DevTools device emulation) que al iniciar captura se activa el modo fullscreen o el fallback CSS — QA OK (`isMobileOrTouch()` + `requestFullscreen()` con fallback fixed implementado)
- [x] 10.5 Verificar que al desmontar el componente (navegar a otra pantalla) no quedan errores en consola — QA OK, `disposeEnrollmentEngine()` cleanup confirmado
