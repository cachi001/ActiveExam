# Spec delta — state-transition-rules (C-35)

> Recalibra los umbrales de gaze en `DEFAULT_CONFIG` a valores alcanzables con el vector iris real, y extiende `FrameSignals` con `head_yaw_deg?` para incorporar head pose como señal complementaria en `evalGaze()`.
