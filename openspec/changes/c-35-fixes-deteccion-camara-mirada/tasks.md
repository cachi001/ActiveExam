...
- [ ] 7.1 **Bug 1 — verificación de cámara limpia**: iniciar el harness (cámara activa y corriendo), navegar a otra ruta (p.ej. `/admin`), volver a `/admin/detection-test`, verificar que el video no muestra frame congelado de la sesión anterior y que el harness arranca en estado `idle` (sin frames hasta presionar Iniciar)
- [ ] 7.2 **Bug 2 — verificación de mirada sostenida**: iniciar el harness con motor real activo, mirar sostenidamente hacia un lado por 3–4 segundos; verificar en el panel de eventos que `mirada_desviada_sostenida` aparece en el log; verificar que el medidor de riesgo (C-33) sube al menos 12 puntos (peso de evento "media" según `riskWeights.ts`)
- [ ] 7.3 **Verificación de no-regresión en Examen.tsx**: confirmar que `Examen.tsx` no depende del valor hardcodeado `0.6` directamente — buscar cualquier referencia a `gaze_deviation_threshold` hardcodeada fuera de `DEFAULT_CONFIG`; si existe, actualizar para usar el valor de `DEFAULT_CONFIG` o el valor de config del backend
