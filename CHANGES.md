...
| **c-71** `inscripcion-gate-y-cola-revision` | **slice 1 hecho** (backend+frontend, sin commitear) | **Change de integridad de examen, por slices.** **Slice 1 — gate de inscripción (implementado + testeado)**: el alumno solo VE y solo RINDE exámenes de comisiones donde está inscripto por código. Catálogo (`listar_examenes_contenido`) y "Mis materias/comisiones" filtrados **server-side por rol** (staff ve todo); backstop **403 `no_inscripto`** en `crear_sesion` (cliente no confiable). Cierra el agujero que c-70 dejó abierto. Extra: seed matricula EST-00x a C1 (source+prod); se quitó el paso de consentimiento redundante del flujo de rendición (el perfil es el único gate). Tests: 8 nuevos + fix de 1 de c-69-exam-config + safety-net enforcement, todo verde (DB real). **Slices pendientes**: **2** Cola de Revisión (funcionalidad + mejora de página), **3** Sesiones Grabadas. | c-70 ✓ | ALTO (control de acceso a exámenes) |

**Leer antes**: `openspec/changes/c-71-inscripcion-gate-y-cola-revision/` y `c-70-matriculacion-por-codigo/` (proposal/design/specs/tasks), `04_modelo_de_datos.md` §comisión/inscripción, `05_reglas_de_negocio.md` §RN-BIO (embedding = dato sensible, cliente no confiable).
