...
> **c-76-panel-supervision-en-vivo (2026-08-14)**: rediseño de roles del panel de supervisión (elimina `PROCTOR`, tutor acotado por comisión) + UX (alta de usuario, notas, sticky). Bloques 1-3, 7, 11, 12 mergeados en `f82a855`. Esta sesión cerró los bloques **4** (límite configurable de pausas, default 2, migración `0069`), **6** (chat/pausa actor `tutor`; el alumno no puede iniciar el hilo), **8** (supervisión del tutor acotada por `asignar_docente`, coordinador/revisor global) y **13** (housekeeping: validate strict OK, Open Questions actualizadas en `design.md`). Bloque **5** (screenshots de pausa) quedó **parcial**: el backend persiste/re-hashea y emite la señal de ausencia (`pausa_sin_captura`), pero el cliente todavía no emite explícitamente el evento `captura_pausa` con cadencia propia durante la ventana de pausa. Bloque **9** (rediseño visual del detalle de sesión) quedó **parcial**: se corrigió el gap de gobernanza (el tutor ya NO ve el botón de veredicto, antes visible aunque el backend lo rechazara con 403) pero no se hizo el rediseño visual completo de dos estados (con/sin riesgo). Ver `openspec/changes/c-76-panel-supervision-en-vivo/tasks.md` para el detalle tarea por tarea. Rama: `feat/c-76-panel-supervision-en-vivo` (sin commitear al cierre de esta sesión).
