"""012 - tabla audit_log append-only con hash encadenado (rama activeexam).

Revision ID: 0012
Revises: 0011 (branch activeexam)
...
PROPOSITO:
  ...
  tabla la crea SOLO la rama principal/full en 0002_core_models.py; la rama activeexam
  (0005 -> 0008 -> ... -> 0011) NUNCA la creo. En el deployment real (activeexam /
  Railway, postgres:16-alpine, sin TimescaleDB) el append a audit_log revienta
  con HTTP 500 porque la tabla no existe.

  Esta migracion porta a la rama activeexam la MISMA tabla + los 2 triggers de 0002,
  ...
  DIFERENCIA vs 0002 (unica y obligatoria): la rama activeexam NO tiene tabla
...
ROLLBACK:
  alembic downgrade activeexam@0011  -> dropea triggers, funciones y la tabla audit_log
  (espeja el downgrade de audit_log en 0002, sin tocar consentimiento/evidencia,
  que no existen en activeexam).

VERIFICACION:
  alembic upgrade activeexam@head    -> aplica 0011 -> 0012 contra postgres:16-alpine.
...
def upgrade() -> None:
    ...
    # en la rama activeexam): evidencia_id queda como UUID nullable SIN FK.
    op.create_table(
        "audit_log",
...
def downgrade() -> None:
    # Espeja el downgrade de audit_log en 0002 (sin tocar consentimiento/evidencia,
    # inexistentes en activeexam): triggers -> funciones -> tabla.
