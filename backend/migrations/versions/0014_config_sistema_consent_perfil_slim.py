"""014 - configuracion_sistema (singleton) + consentimiento_perfil (rama slim).

Revision ID: 0014
Revises: 0013 (branch slim)
...
PROPOSITO (change configuracion-sistema-funcional, Grupo 0):
  Crea DOS tablas ADITIVAS en la rama slim (Railway prod, postgres:16-alpine):
  ...
  Ambas tablas tienen el MISMO schema en full y slim (no usan TimescaleDB). Aditivas:
  sin DROP/ALTER destructivo. El down (paso 2 destructivo) queda separado.

ROLLBACK:
  alembic downgrade slim@0013  -> dropea las dos tablas (destructivo, paso 2).

VERIFICACION:
  alembic upgrade slim@head    -> aplica 0013 -> 0014 contra postgres:16-alpine.
