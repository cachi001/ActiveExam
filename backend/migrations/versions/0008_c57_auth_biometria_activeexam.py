"""008 - auth, usuarios y biometria para el modulo activeexam (Railway / Postgres estandar).

Revision ID: 0008
Revises: 0005 (branch activeexam - NO depende de 0001..0007 de la rama principal)
Create Date: 2026-06-07

RAMA: activeexam
  down_revision = "0005"   <- solo la rama activeexam, no la principal
  branch_labels = None     <- ya esta en la rama activeexam por herencia de 0005
  depends_on    = None     <- cero dependencia de la rama principal

PROPOSITO:
  Crea las 4 tablas que el activeexam necesita para auth JWT propia + enrollment
  ...
  solo existe la rama activeexam; en produccion full solo existe la rama principal.
...
ROLLBACK:
  alembic downgrade activeexam@0005  -> elimina las 4 tablas en orden inverso.
  Las tablas de proctoring (0005) permanecen intactas.

VERIFICACION:
  alembic upgrade activeexam@head   -> aplica 0005 + 0008 contra postgres:16-alpine
  alembic history             -> debe mostrar 0005 -> 0008 en la rama activeexam
"""
...
revision = "0008"
down_revision = "0005"   # rama activeexam - NO la rama principal
branch_labels = None     # ya hereda el label "activeexam" de 0005
...
def upgrade() -> None:
    """Crea las 4 tablas de auth y biometria en Postgres estandar (activeexam).
    ...
    # ------------------------------------------------------------------
    # 3. foto_referencia (activeexam: BYTEA en DB, sin MinIO)
    # ------------------------------------------------------------------
    op.create_table(
        "foto_referencia",
        sa.Column(
