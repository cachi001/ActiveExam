"""Servicio de cadena de custodia — etapa 2 backend + worker etapas 3/4 (C-12).

ETAPA 2 (sincrona, ``EvidenceCustodyService.recibir_notificacion``):
1. Valida la **firma HMAC del cliente** sobre el hash del clip con la clave de sesion
   rotativa (zero trust, RN-GLB-01); firma invalida -> RECHAZO.
...
firma maestra asimetrica de Vault) y etapa 4 (re-inferencia server-side firmada).
Divergencia en el worker -> mismo evento critico. El SLO E2->E4 < 30 s se instrumenta
en la presentacion/worker (Prometheus); aqui la logica es independiente del reloj.
    ...
    async def recibir_notificacion(
        ...
        *,
        ...
        ev = Evidencia(
            ...
            meta={"exam_id": notif.exam_id, "object_key": notif.object_key},
        )
...
class EvidenceSigningWorker:
    ...
    Consume de la cola ganadora de C-03 (vía ``MessageQueuePort``) detras del puerto;
    re-descarga el clip del WORM, re-verifica el hash (3.a), firma con la clave maestra
    asimetrica (Vault) y firma el output de la re-inferencia server-side."""
