"""Application service: GuardarFotoPerfilActiveExamService (c-57, D6).

Servicio de aplicacion para persistir la foto de perfil del alumno en el
modulo activeexam (Railway / Postgres estandar, sin MinIO).
...
Diferencia con ``GuardarFotoPerfilService`` (full):
  ...
  - GuardarFotoPerfilActiveExamService: recibe solo la session, persiste el BYTEA
    directamente en DB via ``DbPhotoStorageService``.

DISENO (D6): se crea un servicio activeexam paralelo en lugar de contaminar el
servicio del full con logica del activeexam. El enrollment router detecta el tipo
...
class GuardarFotoPerfilActiveExamService:
    """Orquesta la persistencia de la foto de perfil del alumno en el activeexam.
