"""Fabricas de ``VerifyFn`` para ``JwtValidator`` (infraestructura, C-06 D2).

Dos implementaciones de la verificacion criptografica:

- ``build_rs256_verify``: PRODUCCION. Usa PyJWT + la JWK (RS256) del JWKS de
  Keycloak. El import de PyJWT es PEREZOSO (dentro de la fabrica): si la lib no
  esta instalada, falla EXPLICITO al construir el verificador, no al importar el
  modulo — asi el resto de auth y los tests con verificador simetrico funcionan
  sin la dependencia.
- ``build_hs256_verify``: SOLO TEST. Verifica una firma simetrica HMAC-SHA256 con
  ``hmac``/``hashlib`` de la stdlib (sin PyJWT, sin red). Permite testear todo el
  flujo de validacion (firma valida/invalida, expiracion, audiencia) sin el stack.

Ambas levantan ``UnauthenticatedError`` (-> 401) ante cualquier fallo.
"""
