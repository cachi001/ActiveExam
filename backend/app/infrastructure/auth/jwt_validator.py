...
Flujo original (C-06, RS256 generico):
...
Flujo multi-issuer (C-55, HS256 propio + RS256 de un segundo issuer opcional):
...
2. Despacha al ``verify_fn`` correcto:
   ...
   - alg=RS256 + iss=RS256_ISSUER   → ``verify_fn_rs256`` (JWKS cacheado)
...
class JwtValidator:
    ...
    - Multi-issuer (C-55): dos ``verify_fn`` (HS256 propio + RS256 de un segundo issuer opcional).
    ...
    def __init__(
        ...
        *,
        ...
        rs256_issuer: str | None = None,
    ) -> None:
        self._jwks = jwks_cache
        self._policy = policy
        self._verify_rs256 = verify_fn  # fn RS256 (segundo issuer opcional, o legacy)
        self._verify_hs256 = verify_fn_hs256  # fn HS256 (provider propio, C-55)
        self._own_issuer = own_issuer
        self._rs256_issuer = rs256_issuer
    ...
    def validar(self, token: str) -> AuthenticatedPrincipal:
        ...
        if self._verify_hs256 is not None:
            ...
            elif alg == "RS256" and iss == self._rs256_issuer:
                # Token del segundo issuer (RS256): verificar con JWKS cacheado (comportamiento C-06).
                jwk = self._jwks.get_jwk(kid) if kid else None
                claims = self._verify_rs256(
                    token, jwk, self._policy.audience, self._rs256_issuer or ""
                )
            else:
