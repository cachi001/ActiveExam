"""Composicion del JwtValidator HS256-only para el modulo activeexam (c-57, D3).

``build_activeexam_jwt_validator`` construye un ``JwtValidator`` que SOLO valida
...
Diferencia con ``wiring.py`` (full):
  ...
  - activeexam_wiring.py: importa ``ActiveExamSettings`` (solo las vars del activeexam), arma
...
es None, lo que no ocurre en el activeexam). Para cumplir el contrato del tipo sin
...
def _stub_jwks_fetch() -> dict:
    """Fetch stub para el JwksCache del activeexam.
    ...
    algun bug de regresion el validador intentara resolver RS256 en el activeexam,
    falla de forma ruidosa y detectable (no silenciosa).
    """
    raise NotImplementedError(
        "El JwksCache del modulo activeexam no debe fetchear JWKS: "
        "el activeexam es HS256-only y no tiene Keycloak. "
        "Si ves este error, hay una regresion en el despacho de alg/iss."
    )


def build_activeexam_jwt_validator(settings: ActiveExamSettings) -> JwtValidator:
    """Construye el JwtValidator HS256-only para el modulo activeexam.

    Args:
        settings: ActiveExamSettings ya inicializado con jwt_own_secret,
    ...
    """
    # Stub de JwksCache (RS256 nunca se usa en el activeexam — ver OQ-1 en design.md).
    ...
    #   iss==keycloak_issuer; como rs256_issuer=None, eso NUNCA ocurre.
    ...
    return JwtValidator(
        ...
        rs256_issuer=None,  # Keycloak no existe en el activeexam
    )


def _rs256_desactivado(token: str, jwk: dict | None, audience: str, issuer: str) -> dict:
    """VerifyFn RS256 desactivado para el activeexam.

    En modo multi-issuer con rs256_issuer=None, esta funcion NUNCA se llama
    (el despacho en JwtValidator solo invoca verify_fn si alg==RS256 y
    iss==keycloak_issuer; rs256_issuer=None nunca coincide con un iss real).
    ...
    raise UnauthenticatedError(
        "RS256 no esta soportado en el modulo activeexam (solo HS256). "
