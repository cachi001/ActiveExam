"""Test de la migración 0032 activeexam (C-69): configuración del examen POR EXAMEN.

Verifica con alembic REAL contra Postgres (postgres:16, sin TimescaleDB):
- `alembic upgrade activeexam@head` aplica hasta 0032: examen_contenido gana las 7
  ...
  a 100/60 (escala 0-100, nunca "sobre 10") — este test corre contra activeexam@head,
  así que verifica el default VIGENTE, no el de 0032 en aislamiento.
- `downgrade activeexam@0031` dropea las 7 columnas, dejando intacto examen_contenido.
...
def test_migracion_0032_agrega_config_con_defaults() -> None:
    ...
    _alembic(["downgrade", "activeexam@0031"])
    ...
    # Defaults (vigentes en activeexam@head, post-migración 0061)
