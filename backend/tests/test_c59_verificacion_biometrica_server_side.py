...
def _crear_usuario_activeexam_sync(username: str, password: str, roles: list[str]) -> str:
    """Crea un usuario en la DB activeexam y devuelve su id (str UUID)."""
    ...
    async def _crear() -> str:
        ...
        db_url = _normalizar_db_url(_DB_URL_ACTIVEEXAM)
...
def _limpiar_usuario_activeexam_sync(usuario_id: str) -> None:
    """Elimina el usuario y sus embeddings/fotos de la DB activeexam."""
...
def _login_y_token(client, username: str, password: str) -> str:
...
# ---------------------------------------------------------------------------
# Fixture: activeexam_client
# ---------------------------------------------------------------------------


@pytest.fixture
def activeexam_client(monkeypatch: pytest.MonkeyPatch):
    """TestClient del activeexam apuntando a postgres:16-alpine con EMBEDDING_KEY real."""
    import importlib
    ...
    activeexam_app = main_activeexam_module.create_activeexam_app()

    with TestClient(activeexam_app) as c:
        yield c
...
class TestVerificarReferenciaServerSide:
    ...
    def test_embedding_cercano_es_match_true(self, activeexam_client) -> None:
        """Enrollment + verificar-referencia con embedding cercano -> 200 es_match=true."""
        usuario_id = _crear_usuario_activeexam_sync(
            "c59-match-true", "password123", ["estudiante"]
        )
        try:
            token = _login_y_token(activeexam_client, "c59-match-true", "password123")

            # Enrollment del embedding de referencia.
            referencia = _vector_128(0.1)
            _enrollar_embedding(activeexam_client, token, referencia)
            ...
            _limpiar_usuario_activeexam_sync(usuario_id)

    # ------------------------------------------------------------------
    # 4.2 Embedding vivo lejano -> 200 es_match=false
    # ------------------------------------------------------------------

    def test_embedding_lejano_es_match_false(self, activeexam_client) -> None:
        """Embedding vivo lejano -> 200 es_match=false (no sancion, solo prioriza)."""
        usuario_id = _crear_usuario_activeexam_sync(
            "c59-match-false", "password123", ["estudiante"]
        )
        try:
            token = _login_y_token(activeexam_client, "c59-match-false", "password123")
    ...
    def test_sin_referencia_vigente_retorna_404(self, activeexam_client) -> None:
        """Usuario autenticado sin embedding vigente -> 404."""
        usuario_id = _crear_usuario_activeexam_sync(
            "c59-sin-ref", "password123", ["estudiante"]
        )
        try:
            token = _login_y_token(activeexam_client, "c59-sin-ref", "password123")
    ...
    def test_embedding_dimension_invalida_retorna_422(self, activeexam_client) -> None:
        """Embedding vivo con dimension distinta a la referencia -> 422."""
        usuario_id = _crear_usuario_activeexam_sync(
            "c59-dim-inv", "password123", ["estudiante"]
        )
        try:
            token = _login_y_token(activeexam_client, "c59-dim-inv", "password123")

            # Enrollar referencia 128-d.
            _enrollar_embedding(activeexam_client, token, _vector_128(0.1))
    ...
    def test_campo_embedding_referencia_extra_retorna_422(self, activeexam_client) -> None:
        """Body con embedding_referencia extra -> 422 por extra='forbid'."""
        usuario_id = _crear_usuario_activeexam_sync(
            "c59-extra-field", "password123", ["estudiante"]
        )
        try:
            token = _login_y_token(activeexam_client, "c59-extra-field", "password123")
    ...
    def test_sin_bearer_retorna_401(self, activeexam_client) -> None:
    ...
    def test_rol_incorrecto_retorna_403(self, activeexam_client) -> None:
        ...
        try:
            token = _login_y_token(activeexam_client, "c59-rol-coordinador", "password123")
    ...
    def test_estado_referencia_true_con_referencia(self, activeexam_client) -> None:
        """GET referencia/estado -> tiene_referencia_vigente=true si hay embedding."""
        usuario_id = _crear_usuario_activeexam_sync(
            "c59-estado-true", "password123", ["estudiante"]
        )
        try:
            token = _login_y_token(activeexam_client, "c59-estado-true", "password123")
            _enrollar_embedding(activeexam_client, token, _vector_128(0.1))
    ...
    def test_estado_referencia_false_sin_referencia(self, activeexam_client) -> None:
        """GET referencia/estado -> tiene_referencia_vigente=false si no hay embedding."""
        usuario_id = _crear_usuario_activeexam_sync(
            "c59-estado-false", "password123", ["estudiante"]
        )
        try:
            token = _login_y_token(activeexam_client, "c59-estado-false", "password123")
    ...
    def test_estado_referencia_sin_bearer_401(self, activeexam_client) -> None:
        """GET referencia/estado sin Bearer -> 401."""
        resp = activeexam_client.get("/api/v1/proctoring/biometria/referencia/estado")
        assert resp.status_code == 401, f"Esperado 401, got {resp.status_code}: {resp.text}"
    ...
    def test_embedding_referencia_no_en_respuesta_verificacion(self, activeexam_client) -> None:
        """La respuesta de verificar-referencia NO contiene el embedding descifrado."""
        usuario_id = _crear_usuario_activeexam_sync(
            "c59-no-emb-resp", "password123", ["estudiante"]
        )
        try:
            token = _login_y_token(activeexam_client, "c59-no-emb-resp", "password123")
            referencia = _vector_128(0.1)
            _enrollar_embedding(activeexam_client, token, referencia)
    ...
    def test_endpoint_demo_verificar_eliminado(self, activeexam_client) -> None:
