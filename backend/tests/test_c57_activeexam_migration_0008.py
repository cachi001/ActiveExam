"""Test de la migracion 0008 activeexam (c-57, task 1.7).

Verifica que:
- alembic upgrade activeexam@head en postgres:16-alpine aplica 0005 + 0008 sin error.
...
Requiere RUN_STACK_TESTS=1 con DATABASE_URL_ACTIVEEXAM apuntando a postgres:16-alpine.
"""
...
# DATABASE_URL_ACTIVEEXAM puede ser asyncpg o sync. Normalizamos a sync (psycopg2).
_DB_URL_ACTIVEEXAM_RAW = os.environ.get(
    "DATABASE_URL_ACTIVEEXAM",
    os.environ.get("DATABASE_URL_SYNC_ACTIVEEXAM", "postgresql://app@db-activeexam:5432/proctoring"),
)
...
_DB_URL_SYNC_ACTIVEEXAM = _to_sync(_DB_URL_ACTIVEEXAM_RAW)


@pytest.mark.requires_stack
def test_migracion_0008_activeexam_upgrade_crea_tablas(
    monkeypatch: pytest.MonkeyPatch,
) -> None:
    """activeexam@head en postgres:16-alpine aplica 0005 + 0008, crea las 7 tablas."""
    import subprocess
    import sys
    import os
    ...
    def run(args: list[str]) -> None:
        env = {**os.environ, "DATABASE_URL": _DB_URL_SYNC_ACTIVEEXAM}
    ...
    # Aplicar activeexam@head (0005 + 0008).
    run(["upgrade", "activeexam@head"])

    import psycopg2

    conn = psycopg2.connect(_DB_URL_SYNC_ACTIVEEXAM)
    cur = conn.cursor()
    ...
    # Verificar columna foto_bytes en foto_referencia (activeexam: BYTEA, sin uri_storage).
    cur.execute(
    ...
    assert "uri_storage" not in columnas_foto, (
        "foto_referencia tiene uri_storage (columna del full, no del activeexam)"
    )
...
@pytest.mark.requires_stack
def test_migracion_0008_activeexam_downgrade_revierte_tablas() -> None:
    """activeexam downgrade a 0005 elimina las 4 tablas de auth+biometria, proctoring intacto."""
    import subprocess
    import sys
    import os
    ...
    # Volver a activeexam@head para no dejar la DB rota.
    run(["upgrade", "activeexam@head"])
