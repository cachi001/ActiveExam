"""Tests unitarios: activeexam_wiring.py — JwtValidator HS256-only (c-57, task 3.6).

Verifica que:
  - Token HS256 valido con jwt_own_secret es aceptado.
  - Token con secreto incorrecto lanza UnauthenticatedError.
  - Token RS256 (no soportado en activeexam) lanza UnauthenticatedError.
...
def _make_settings(secret: str = "super-secret-min-32-bytes-long-key-here"):
    """Crea ActiveExamSettings minimas para los tests de wiring."""
...
class TestBuildActiveExamJwtValidator:
    """Tests del JwtValidator HS256-only construido por activeexam_wiring."""
    ...
    def test_token_hs256_valido_es_aceptado(self) -> None:
        ...
        validator = build_activeexam_jwt_validator(settings)

        token = _token_hs256(secret, _claims_validos())
        principal = validator.validar(token)

        assert principal.username == "seed-estudiante"
    ...
    def test_token_con_issuer_incorrecto_es_rechazado(self) -> None:
        ...
        # Token con issuer de Keycloak (no aceptado en el activeexam).
        token = _token_hs256(
            secret,
    ...
    def test_jwks_stub_nunca_se_llama_en_hs256(self) -> None:
        """El JwksCache stub del activeexam no se llama en el path HS256 normal."""
        from app.infrastructure.auth.activeexam_wiring import build_activeexam_jwt_validator
