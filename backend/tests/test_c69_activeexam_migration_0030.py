"""Test de la migración 0030 activeexam (C-69, D12 parte B): destino Moodle por examen.

Verifica con alembic REAL contra Postgres (postgres:16, sin TimescaleDB):
- `alembic upgrade activeexam@head` aplica hasta 0030: examen_contenido gana las
  columnas NULLABLE moodle_courseid (integer) y moodle_cmid (integer).
- `downgrade activeexam@0029` dropea ambas columnas, dejando intacto examen_contenido.
...
def test_migracion_0030_downgrade_revierte() -> None:
    _alembic(["downgrade", "activeexam@0029"])
