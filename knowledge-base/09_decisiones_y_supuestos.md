...
### DD-20 (Decisión de integración) — Integración con LMS: LTI 1.3 universal **+ plugin Moodle `quizaccess`** (decisión revisada 2026-06-11; **revisada en parte por C-69 el 2026-06-28**)

> **⚠️ Revisión C-69 (2026-06-28) — la plataforma AHORA opera el examen (Moodle import + lockdown + write-back de nota).**
> **Estado**: vigente; **supera en parte** la premisa original de "rendir alrededor" (ver abajo, marcada como ~~original~~). El resto de DD-20 (capa LTI universal + plugin `quizaccess` como evolución) sigue en pie.
> **Decisión (C-69)**: para el LMS primario (Moodle) la plataforma **opera el examen end-to-end**, no se limita a proctorear alrededor:
> 1. **Importa el contenido del examen desde Moodle** vía **Moodle XML export** (formato nativo de banco de preguntas de Moodle). El examen deja de vivir solo en el LMS: se ingiere en la plataforma.
> 2. El **estudiante rinde DENTRO de la plataforma** (interfaz propia de preguntas/respuestas), bajo **lockdown a pantalla completa** (fullscreen) — el examen y el proctoring viven en la misma superficie, sin saltos a la actividad del LMS.
> 3. La plataforma **devuelve la nota académica a Moodle** vía web service REST **`core_grades_update_grades`** (llamada **server-side**, cadena de custodia preservada). La nota se calcula en la plataforma y se escribe de vuelta al libro de calificaciones de Moodle.
> **Estado del retorno de nota (hoy)**: el write-back es por **REST WS** (`core_grades_update_grades`). **LTI 1.3 + AGS** (Assignment and Grade Services) **queda como evolución futura** — es la ruta estándar/universal de retorno de score descrita en el cuerpo de DD-20, pero hoy NO está implementada; el camino productivo es el REST WS de Moodle.
> **Justificación**: operar el examen en plataforma permite el lockdown fullscreen real y la integración fluida con el pipeline de proctoring (re-inferencia, evidencia, scoring) sin depender de incrustarse en el quiz nativo de Moodle. El XML export + REST WS son la vía más directa contra Moodle hoy; LTI 1.3 + AGS amortizan a futuro la cobertura multi-LMS.
> **Materializado en**: change `c-69` (examen en plataforma con Moodle + lockdown + write-back de nota) — revive el alcance de `c-44` (creación/operación de examen en plataforma), originalmente cancelado.

**Decisión (original 2026-06-11)**: la integración con el sistema de evaluación institucional (Moodle, Canvas, Blackboard, D2L, Sakai…) se hace por **dos capas complementarias** (decisión revisada el 2026-06-11, materializada en el change futuro `integracion-lms-lti` — sin número aún):
1. **LTI 1.3** (1EdTech/IMS Global) como **capa universal** — UN desarrollo integra con cualquier LMS compatible. Lanzamiento LTI → handshake de identidad y contexto → proctoring envolviendo la actividad del examen. Identidad encaja con el OIDC de Keycloak (DD-09); roster vía **NRPS** (Names and Roles Provisioning Service); retorno de resultado/score vía **AGS** (Assignment and Grade Services).
2. **Plugin Moodle `quizaccess` nativo** como **capa profunda para Moodle** (LMS primario institucional) — mete el proctoring como **regla de acceso al quiz nativo**: gate de cámara/consentimiento antes de iniciar el intento + monitoreo DURANTE el quiz, sin saltos de pantalla. Es la integración fluida que LTI puro no da (con LTI puro el proctoring es una actividad separada, no se incrusta en el quiz existente).

~~En ambas capas el LMS **opera el examen** (preguntas, respuestas, nota); el proctoring NO lo crea ni lo importa, se integra **alrededor**.~~ *(superado en parte por la Revisión C-69, arriba: para Moodle la plataforma SÍ importa el examen — Moodle XML export — el alumno lo rinde en plataforma con lockdown, y la nota vuelve a Moodle por REST WS `core_grades_update_grades`.)* **Se acepta el costo del doble mantenimiento** (Tool Provider LTI + plugin PHP de Moodle) y el lock-in parcial del plugin a Moodle, a cambio de la UX fluida en el LMS primario.
**Modelo de configuración por examen**: la **info del examen** (nombre, cátedra, horario, duración) la **provee el LMS** (contexto del launch LTI / roster NRPS) — el proctoring NO la crea ni la importa. Los **parámetros de proctoring** (detectores activos, umbral de revisión, días de retención Ley 25.326) son **propios del proctoring** y se configuran **por examen, asociados al contexto del LMS** (una vez conectado, el docente/admin elige el perfil de proctoring para ese examen). La UI de configuración existente (`ConfigureExam.tsx`, change `c-44`) se **reaprovecha re-enganchada al contexto LTI**: muere el bloque "info del examen" (lo da el LMS), sobrevive el bloque "parámetros de proctoring". La creación local de exámenes queda como **demo/fallback** (institución sin LMS), no como camino principal.
...
**Justificación**: LTI 1.3 es el estándar universal — UN solo desarrollo integra con cualquier LMS compatible (respuesta directa al requisito de **adaptación fácil a cualquier entidad académica**). El plugin Moodle `quizaccess` se suma porque Moodle es el **LMS primario institucional** y se requiere la experiencia sin fricción dentro del quiz nativo, que LTI puro no provee. No son excluyentes: LTI cubre el universo de LMS, el plugin profundiza Moodle.
**Alcance temporal**: la implementación es **Fase 2** (FR-17 / change futuro `integracion-lms-lti` — re-incorporado al roadmap pero aún sin crear en el CLI ni asignado a un número específico; el número `C-49` del plan original lo tomó otro change ya archivado, y se le asignará el siguiente número libre del CLI al proponerlo), depende de los gates C-01 (DPIA) + C-02 (revisores) + del MVP operativo (no se integra un proctoring que aún no existe). **Accionable desde el MVP**: diseñar la superficie de API "LTI-ready" — endpoint/webhook de retorno de resultado (AGS), provisioning de roster (NRPS) y mapeo de claims→roles (hoy huecos: la API REST y el flujo de lanzamiento son propios, sin handoff externo).
**Alternativas consideradas**: A) **LTI 1.3** universal; B) plugin Moodle `quizaccess` (profundo pero solo Moodle); C) iframe embedding sin estándar (frágil); D) flujo coordinado custom por LMS (no escala). **Decisión adoptada: A + B en conjunto** (LTI universal + plugin Moodle profundo), no una sola.
**Trade-offs (aceptados)**: doble superficie de integración a construir y mantener (Tool Provider LTI 1.3 — OIDC login, deep linking, AGS, NRPS — **y** plugin PHP `quizaccess` de Moodle) y lock-in parcial del plugin a Moodle. Se aceptan a cambio de: cobertura universal de LMS (LTI) + experiencia sin fricción en el LMS primario (plugin). El LTI se amortiza desde la segunda institución; el plugin se justifica por el volumen en Moodle.
