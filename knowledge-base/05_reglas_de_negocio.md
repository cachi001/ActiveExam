...
- **RN-CC-01**: Solo los eventos de severidad alta o crítica disparan captura automática de evidencia (clip de 5–10 s).
- **RN-CC-02**: La cadena de custodia tiene cuatro etapas criptográficas **acumulativas** (las firmas no se reemplazan, se encadenan): (1) cliente: SHA-256 + firma HMAC con clave de sesión rotativa; (2) backend al recibir: valida firma cliente, recalcula hash, persiste metadata, deposita en bucket + audit log; (3) worker asíncrono: re-descarga, 3.ª verificación de hash, firma con clave maestra asimétrica (RSA-2048 / Ed25519); (4) re-inferencia opcional: output del modelo firmado por el backend sobre el clip exacto.
...
- **RN-RV-04**: El revisor accede al contexto completo: línea de tiempo de eventos, clips firmados, observaciones del proctor en vivo, output de re-inferencia server-side y audit log de accesos previos.
